...
interface HttpServer {
  ...
  onJsonRequest<T>(handler: (req: T, res: (status: number, data?: any) => void) => void): void,
}
...
const attemptStart = (port = 8001, srv = createServer()): Promise<HttpServer> => new Promise((fin, fail) => {
  srv.on('error', e => port < 65536
