...
const doStripListLeaders = (text: string, listUnicodeChar: boolean) => listUnicodeChar
  ? text.replace(/^([\s\t]*)([\*\-\+]|\d+\.)\s+/gm, `${listUnicodeChar} $1`)
  : text.replace(/^([\s\t]*)([\*\-\+]|\d+\.)\s+/gm, '$1')

const doGithubMarkdown = (text: string) => text
  .replace(/\n={2,}/g, '\n') // header
...
export const remove = (markdown = '', options = {} as MarkdownOptions) => {
  ...
  try {
    const out2 = stripListLeaders ? doStripListLeaders(out1, listUnicodeChar) : out1
    ...
    return out3
      ...
      .replace(/^(\n)?\s{0,}#{1,6}\s+| {0,}(\n)?\s{0,}#{0,} {0,}(\n)?\s{0,}$/gm, '$1$2$3') // atx-style headers
