...
const findInNextPath = <T extends LocationItem>(items: T[], findNext: boolean) => {
  const sortedItems = sortItems(items)
  return findNext
    ? sortedItems[0]
    : sortedItems[sortedItems.length - 1]
...
const findClosest = <T extends LocationItem>(
  ...
  const nextPathItems = currentPathItems.length ? [] : items.filter(m => m.path === items[0].path)
