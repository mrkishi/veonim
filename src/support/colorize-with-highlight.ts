...
const markAsHighlight = (colors: ColorData[], start: number, end: number) => colors.reduce((res, m, ix) => {
  const prev: ColorData = res[res.length - 1]
  const charIsHighlighted = ix >= start && ix <= end
  const reusePreviousGroup = prev && prev.color === m.color && prev.highlight === charIsHighlighted
...
export default (data: ColorizedFilterResult, highlightColor?: string) => highlightColor
  ? recolor(data.colorizedLine, data.start.column, data.end.column, highlightColor)
