...
export default class extends Transform {
  ...
  superparse(raw: Buffer) {
    ...
    // uint32
      ...
      const val = (raw[this.ix + 1] * 16777216) + (raw[this.ix + 2] << 16) + (raw[this.ix + 3] << 8) + raw[this.ix + 4]
      this.ix += 5
      return val
    }
    ...
    // int32
      ...
      const val = (raw[this.ix + 1] << 24) | (raw[this.ix + 2] << 16) | (raw[this.ix + 3] << 8) | raw[this.ix + 4]
      this.ix += 5
      return val
    }
  ...
  _transform(chunk: Buffer, _: any, done: Function) {
    ...
    this.skipStringAllocationBecauseMsgpackIsFuckingSlow = redrawEventKey.equals(part)
